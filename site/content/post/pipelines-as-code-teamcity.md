...
You can still rely on TeamCity interface to confirm everything is correct. But the code is simple and concise enough to look and understand how your build is behaving.  
...
_Inspection_ runs basic and fast tests, such as unit tests and linting, and give a green status that a pull request is eligible for merging. Once merged, changes trigger a _Build Release_, that performs the same inspection and additional integration tests, producing a deployable artifact.  
